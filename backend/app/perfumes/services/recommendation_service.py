...
class RecommendationService:
    ...
    @traceable(run_type="chain", name="Hybrid Recommendation")
    ...
    @traceable(run_type="retriever", name="Pinecone RAG Search")
    def _search_pinecone(self, query_text, top_k=50):
        ...
        try:
            ...
            # [LangSmith] 토큰 사용량 및 예상 비용(USD) 명시적 기록
            # text-embedding-3-small 비용: $0.02 / 1M tokens
            run_tree = get_current_run_tree()
            if run_tree and hasattr(response, "usage"):
                usage = response.usage
                estimated_cost = (usage.total_tokens / 1_000_000) * 0.02
